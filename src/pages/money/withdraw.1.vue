...
<template>
  <div class="money-withdraw">
    ...
    {{JSON.stringify(this.bank.name)}}
    <wux-cell-group title="提取现金">
      <wux-cell hover-class="none">
        <wux-input :value="form.name" :controlled="true" label="金额" placeholder="请填写提现金额" @change="(e)=>{this.form.name=e.mp.detail.value }" />
      </wux-cell>
    </wux-cell-group>
    <wux-cell-group title="请填写持卡人本人的银行卡信息">
      ...
      <wux-cell hover-class="none">
        <wux-input :value="form.number" :controlled="true" label="卡号" placeholder="请填写银行卡号" @change="(e)=>{this.form.number=e.mp.detail.value }" />
      </wux-cell>
      <wux-cell hover-class="none">
